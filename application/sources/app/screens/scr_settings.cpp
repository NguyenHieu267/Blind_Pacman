...
view_dynamic_t dyn_view_settings = { { .item_type = ITEM_TYPE_DYNAMIC, }, view_scr_settings };
view_screen_t scr_settings = { &dyn_view_settings, ITEM_NULL, ITEM_NULL, .focus_item = 0 };
...
void scr_settings_handle(ak_msg_t *msg) {
    ...
    case AC_DISPLAY_BUTON_MODE_PRESSED:
        ...
        if (set_cursor == 0) set_difficulty = (set_difficulty < 2) ? set_difficulty + 1 : 0;
        else if (set_cursor == 1) set_sound_on = !set_sound_on;
        else if (set_cursor == 2) {
            if (set_time_limit == 0) set_time_limit = 10;
            else { set_time_limit += 10; if (set_time_limit > 150) set_time_limit = 0; }
        }
        else if (set_cursor == 3) { pm_save_settings(); SCREEN_TRAN(scr_menu_handle, &scr_menu); }
        break;
    }
}